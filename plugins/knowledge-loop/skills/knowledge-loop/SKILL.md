...
(by folder-scoped vector search) when a future session starts or asks.
...
Keep notes short (1-3 sentences), one insight each, no secrets. Skip the trivial.

The `/recall` and `/learn` commands wrap these two steps for on-demand use.
...
- `reference/how-it-works.md` — the capture→store→retrieve loop, folder scoping, and the
  TF-IDF / cosine vector search (and how to swap in embeddings for semantic search).
