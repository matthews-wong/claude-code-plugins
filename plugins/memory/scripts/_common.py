#!/usr/bin/env python3
"""Shared helpers for the unified `memory` plugin (notes + instincts).

Pure Python 3, standard library only — these run straight from Claude Code hooks
with no pip installs. Everything here is deliberately dependency-free.

This module carries the note-side machinery merged in from the original
knowledge-loop plugin: tokenization, a hand-rolled TF-IDF vectorizer + cosine,
near-duplicate merging, confidence math, folder scoping, and atomic writes. The
instinct-side model (support/confidence, Jaccard dedup, promotion) lives in
`memory.py`, which imports these helpers.

Both stores live under ONE directory, `.claude/memory/`:
  * notes.jsonl     — episodic + semantic learnings
  * instincts.jsonl — promoted, durable rules

Backward compatibility: older notes may predate the newer fields (`kind`,
`importance`, `access_count`, `last_used`, `confidence`). Every reader here
defaults those fields, so an older store keeps working unchanged.
"""
...
# --- shared knobs (kept here so all callers agree) -------------------------
...
def memory_dir(root):
    """The single store directory shared by notes + instincts."""
    return os.path.join(root, ".claude", "memory")


def store_path(root):
    """Path to the episodic/semantic learnings store."""
    return os.path.join(memory_dir(root), "notes.jsonl")


def instincts_path(root):
    """Path to the promoted, durable rules store."""
    return os.path.join(memory_dir(root), "instincts.jsonl")
...
def write_notes_atomic(path, notes):
    """Rewrite the whole notes store from `notes`, atomically. Raises on IO failure."""
